...
{
    public class UpdateWhyUseUIDTOValidator : AbstractValidator<UpdateWhyUseDTO>
    {
        public UpdateWhyUseUIDTOValidator()
        {
            RuleFor(x => x.WhyUseId)
                .NotEmpty().WithMessage("WhyUse ID boş olamaz.");
            ...
            RuleForEach(x => x.Items).ChildRules(item =>
            {
                item.RuleFor(x => x.Content)
                    .NotEmpty().When(x => !x.IsDeleted) 
                    .WithMessage("Alt Madde içeriği boş bırakılamaz.")
